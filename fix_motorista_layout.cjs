...
const targetStr = `<div className="mt-1 flex justify-center px-4 py-3 border-2 border-gray-300 border-dashed rounded-md hover:border-gray-400 transition-colors bg-gray-50/50">
 ...
 <p className="text-xs text-gray-500">PNG, JPG, GIF atǸ 10MB</p>
 </div>`;
...
if (motoristaContent.includes(targetStr)) {
    motoristaContent = motoristaContent.replace(targetStr, replaceStr);
    fs.writeFileSync(motoristaPath, motoristaContent);
    console.log('Motorista.tsx patched (restored Alterar/Excluir layout)');
} else {
    console.log('Target string not found in Motorista.tsx');
}
