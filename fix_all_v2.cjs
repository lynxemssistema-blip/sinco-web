...
// 6. Grid 2 header (line 702 roughly)
// Target explicitly the Recurso column in Grid 2 which is right after Seq
const regexGrid2 = /(<th className="p-1\.5 px-2 text-\[9px\] font-bold text-gray-500 uppercase tracking-wide w-8">Seq<\/th>\s*)<th className={colsCls}>Recurso<\/th>/;
const grid2HeaderNew = `$1<th className={colsCls}>\n                      <div className="flex flex-col gap-1">\n                        <span>Recurso</span>\n                        <input type="text" placeholder="Filtro..." value={procTableFiltro} onChange={e => setProcTableFiltro(e.target.value)} className="w-full px-1 py-0.5 text-[9px] font-normal border border-gray-200 rounded focus:outline-none focus:border-teal-500 bg-white" />\n                      </div>\n                    </th>`;
content = content.replace(regexGrid2, grid2HeaderNew);

// 7. Grid 3 header (line 804 roughly)
// Target explicitly the Code and Desc columns that are right after the select column
const regexGrid3 = /(<div className="w-3 h-3 border border-gray-300 rounded-sm mx-auto bg-gray-50" title="Selecione individualmente"><\/div>\s*<\/th>\s*)<th className={colsCls}>C.digo<\/th>\s*<th className={colsCls}>Descri..o<\/th>/;
const grid3HeaderNew = `$1<th className={colsCls}>\n                        <div className="flex flex-col gap-1">\n                          <span>Código</span>\n                          <input type="text" placeholder="Filtro Cód..." value={filtroCod3} onChange={e=>setFiltroCod3(e.target.value)} className="w-full px-1 py-0.5 text-[9px] font-normal border border-gray-200 rounded focus:outline-none focus:border-indigo-500 bg-white" />\n                        </div>\n                      </th>\n                      <th className={colsCls}>\n                        <div className="flex flex-col gap-1">\n                          <span>Descrição</span>\n                          <input type="text" placeholder="Filtro Desc..." value={filtroDesc3} onChange={e=>setFiltroDesc3(e.target.value)} className="w-full px-1 py-0.5 text-[9px] font-normal border border-gray-200 rounded focus:outline-none focus:border-indigo-500 bg-white" />\n                        </div>\n                      </th>`;
content = content.replace(regexGrid3, grid3HeaderNew);

fs.writeFileSync(file, content, 'utf8');
console.log('Restored and applied filters accurately.');
