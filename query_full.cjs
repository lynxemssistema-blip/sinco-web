...
async function testFullRecalculate() {
    ...
    try {
        ...
        `);
        console.log("Full Recalculate OS Success!");
        
        // Also test percentual
        await conn.execute(`
            UPDATE ordemservico os
            SET 
            ...
            WHERE os.IdOrdemServico = 29
        `);
        console.log("Full Percentual Success!");
    } catch(err) {
...
testFullRecalculate().catch(console.error);
