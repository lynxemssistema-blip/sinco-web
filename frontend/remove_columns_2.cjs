...
while(i < lines.length) {
    if (lines[i].includes('<td className="px-2 py-0.5 text-right">') && 
        ...
        lines[i+2] && lines[i+2].includes('<Eye') &&
        lines[i+3] && lines[i+3].includes('</button>') &&
        lines[i+4] && lines[i+4].includes('</td>')) {
        lines.splice(i, 5);
    } else {
        i++;
    }
...
console.log('Removed columns from VisaoGeralPendencias successfully');
