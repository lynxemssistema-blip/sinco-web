...
c4 = c4.replace(/type RomaneioPageProps = \{[^}]*\};\r?\n/g, ''); // remove unused interface
c4 = c4.replace(/useState<any\[\]>/g, 'useState<Record<string, unknown>[]>');
c4 = c4.replace(/useState<any \| null>/g, 'useState<Record<string, unknown> | null>');
...
c5 = c5.replace(/useState<any\[\]>/g, 'useState<Record<string, unknown>[]>');
c5 = c5.replace(/useState<any \| null>/g, 'useState<Record<string, unknown> | null>');
c5 = c5.replace(/as any/g, 'as Record<string, unknown>');
fs.writeFileSync(f5, c5);

console.log('Fixed Lote 5 files - pass 3');
